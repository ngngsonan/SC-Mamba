...
def train_model(config):
    ...
    for epoch in range(initial_epoch, config['num_epochs']):
        ...
        if config.get('diag_prints', False) and batch_idx > 0:
            ...
            print(f"  ║ beta (KL wt) = {beta:.4f}  (annealed)")
            print(f"  ║ gamma (spar) = {gamma:.4f}  (constant)")
            print(f"  ║ avg NLL      = {epoch_nll_sum / batch_idx:7.4f} (lower is better)")
            print(f"  ║ avg KL       = {epoch_kl_sum / batch_idx:7.4f} (lower is better)")
            print(f"  ║ avg KL×beta  = {beta * epoch_kl_sum / batch_idx:7.6f}")
            print(f"  ║ avg Loss     = {full_epoch_accumulated_loss / batch_idx:7.4f} (lower is better)")
            print(f"  ║ avg GradNorm = {epoch_grad_norm_sum / batch_idx:7.4f}")
            print(f"  ║ sigma2 range = [{epoch_sigma2_min:.6f}, {epoch_sigma2_max:.4f}]")
            if epoch_tau_sum > 0 or epoch_sparsity_sum >= 0:
                print(f"  ║ avg Tau      = {epoch_tau_sum / batch_idx:.4f}  (constant) | Alpha = {epoch_alpha_sum / batch_idx:.2f} (learned, lower=sparser)")
                print(f"  ║ avg MaskMean = {epoch_mask_mean_sum / batch_idx:.4f}  (lower is better, dense fraction)")
                print(f"  ║ Mask Sparsity= {epoch_sparsity_sum / batch_idx * 100:5.2f}% (higher is better, % < 0.01)")
                print(f"  ║ S2 Collapse  = {epoch_collapse_sum / batch_idx * 100:5.2f}% (<1e-3, lower is better)")
            print(f"  ║ val_loss     = {avg_val_loss:7.4f} (lower is better)")
            print(f"  ║ best_val     = {best_val_loss:7.4f} (lower is better)")
            print(f"  ╚═══════════════════════════════════════════════════════╝")
